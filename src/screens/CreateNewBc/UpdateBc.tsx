...
const UpdateBc = () => {
  ...
  const updateBc = async (details: any) => {
    ...
    setIsDisabled(true);
    const data = { 
      // membersId: members.map((member: { id: any; }) => String(member.id)),
      title: details.title,
    ...
    <View flex={1} bg={'BACKGROUND_COLOR'} px={horizontalScale(20)}>
      ...
      {!load ? (
        <ScrollView scrollEnabled={scroll} showsVerticalScrollIndicator={false}>
          <FormControl w="100%">
            ...
            <Text
              ...
              fontSize={'sm'}
              color={'GREY'}>
             <Text>Total expected amount per BC: {totalExpected}</Text>
            </Text>
          </FormControl>
          {/*  */}
          <View
        ...
        <ToggleSwitch
          ...
          size="medium"
          onToggle={() => setBalloting(!balloting)}
          thumbOffStyle={{ backgroundColor: Colors.WHITE_COLOR }}
          thumbOnStyle={{ backgroundColor: Colors.WHITE_COLOR }}
        />
      </View>
          </View>
          {openDate && (
